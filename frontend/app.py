...
if os.getenv("USE_GCS", "false").lower() == "true":
    SIMILARITY_API_URL_PRIMARY = "https://art-dna-api-521843227251.europe-west1.run.app/similar"
    SIMILARITY_API_URL_FALLBACK = "http://localhost:8000/similar"
else:
    SIMILARITY_API_URL_PRIMARY = "http://localhost:8000/similar"
    SIMILARITY_API_URL_FALLBACK = "https://art-dna-api-521843227251.europe-west1.run.app/similar"
...
def send_image_to_api(image_bytes, filename, mime, primary_url, fallback_url):
    files = {"image": (filename, io.BytesIO(image_bytes), mime)}  # Package image as a file
    for url in [primary_url, fallback_url]:                        # Try primary first, then fallback
        try:
            ...
            if debug_mode:
                st.warning(f"Failed to reach {url}: {e}")          # Show warning if request fails
...
if uploaded_file is not None:
    ...
    with left_col:
        st.image(uploaded_file, caption="Uploaded Image", use_container_width=True)  # Show uploaded image

    with right_col:
        if st.button("Explore the artistic DNA of your image"):                            # When user clicks "Predict genres"
            ...
            result, used_url = send_image_to_api(
                img_bytes, uploaded_file.name, uploaded_file.type,
            ...
            similar_result, used_sim_url = send_image_to_api(
                img_bytes, uploaded_file.name, uploaded_file.type,
                SIMILARITY_API_URL_PRIMARY, SIMILARITY_API_URL_FALLBACK
            )
